...
import Loading from '../Loading/Loading'; // 👈 import your same spinner
...
const Appspage = () => {
    ...
    useEffect(() => {
        ...
        }, 400); // small delay for smoother UX
    ...
    return (
        <div className='py-15 bg-[#d2d2d233] relative'>
            {/* 👇 show the same spinner while searching */}
            {loading && <Loading />}
            ...
            <div className='my-3 flex justify-between items-center'>
                ...
                <div className='flex justify-items-start items-center border border-gray-400 rounded-lg p-1'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-4 gap-6'>
                {filteredApps.length > 0 ? (
                    filteredApps.map((singleApp) => (
                        <Link to={`/appDetails/${singleApp.id}`} key={singleApp.id}>
                            <div className='card bg-base-100 gap-5 shadow-sm hover:shadow-md transition'>
                                <figure>
                                    <img
